...
server.patch('/pets/{petName}', (req, res, next) => {
    if (req.body.name !== req.params.petName) {
        ...
        server.dataStore.delete(new Resource(req.path), (err, pet) => {
            ...
            server.dataStore.save(new Resource('/pets', req.body.name, pet), (err, pet) => {
                // Send the response
                res.json(pet.data);
            });
        });
...
server.listen(() => {
    console.log('The Swagger Pet Store is now running at http://localhost:8000/pets');
});
